...
class Peminjaman extends CI_Controller {
    ...
    public function datatables()
    {
        ...
        foreach ($list as $field) {
            ...
            $row[] = $field['status'] == 'p' ? '<div class ="btn btn-danger btn-sm disabled">Pinjam</div>' : '<div class="btn btn-success btn-sm disabled">Kembali</div>';
            $row[] = '
                    <a href="' . site_url('detail/read/' . $field['kd_pinjam']) . '" class="btn btn-info btn-sm " title="Lihat detail">
						<i class="fas fa-search"> </i> 
					</a>
					<a href="' . site_url('peminjaman/delete/' . $field['kd_pinjam']) . '" class="btn btn-danger btn-sm btnHapus" title="Hapus">
    ...
    public function insert()
    {
        ...
        $data_pinjam     = $this->peminjaman_model->getLastData();
    ...
    public function insert_submit()
    {
        ...
        if ($this->input->post('submit') == 'Proses') {
            ...
            if ($this->form_validation->run() == TRUE) {
                ...
                $this->session->set_tempdata('message', 'Transaksi berhasil, silahkan pilih buku yang ingin dipinjam!', 1);
